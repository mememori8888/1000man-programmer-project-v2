...
REPO_URL = "https://github.com/mememori8888/1000man-programmer-project-v2"
...
def validate_evidence(payload: dict[str, Any]) -> EvidenceValidationResult:
    ...
    _require_run_id(payload, ("preflight", "run_id"), errors)
    _require_github_url(payload, ("preflight", "summary_url"), r"/actions/runs/\d+$", errors)
    _require_exact_text(payload, ("preflight", "dry_run_artifact"), "work-bigquery-dry-run.json", errors)

    _require_github_url(payload, ("issueops", "issue_url"), r"/issues/\d+$", errors)
    ...
    _require_run_id(payload, ("extract", "run_id"), errors)
    _require_gcs_uri(payload, ("extract", "raw_payload_gcs_uri"), errors)
    _require_gcs_uri(payload, ("extract", "raw_manifest_gcs_uri"), errors)

    _require_run_id(payload, ("load_transform", "run_id"), errors)
    _require_text(payload, ("load_transform", "raw_load_job_id"), errors)

    transform_run_id = _get(payload, ("load_transform", "transform_run_id"))
    if _is_non_empty_text(transform_run_id) and not _is_run_id(transform_run_id):
        errors.append("load_transform.transform_run_id must be a GitHub run id")
    elif not _is_non_empty_text(transform_run_id):
    ...
    if not isinstance(exports, dict):
        ...
        for table in REQUIRED_EXPORT_TABLES:
            if table not in exports:
                ...
                continue
            _require_run_id(payload, ("exports", table, "run_id"), errors)
            _require_gcs_uri(payload, ("exports", table, "gcs_uri"), errors)

    _require_run_id(payload, ("compatibility_audit", "run_id"), errors)
...
def _require_run_id(payload: dict[str, Any], path: tuple[str, ...], errors: list[str]) -> None:
    value = _get(payload, path)
    if not _is_run_id(value):
        errors.append(f"{'.'.join(path)} must be a GitHub run id")
...
def _require_github_url(
    ...
    if not _is_non_empty_text(value):
        errors.append(f"{'.'.join(path)} is required")
        return
    if not value.startswith(REPO_URL) or not re.search(suffix_pattern, value):
        errors.append(f"{'.'.join(path)} must be a {REPO_URL} URL matching {suffix_pattern}")
