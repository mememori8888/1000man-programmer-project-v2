...
Extractor and raw-ingest workflows invoke `elt-raw-write` once with both local and optional GCS destinations. That preserves one manifest, one object name, and one SHA-256 across local artifacts, GCS, and the BigQuery raw row.
...
Compatibility verification is explicit: `.github/workflows/compatibility-audit.yml` loads one legacy CSV from the private data repo into a per-run temporary BigQuery audit table, optionally refreshes the managed mart transforms, then compares row counts, distinct key counts, and key-diff samples against a v2 mart table. When `fail_on_diff` is enabled, the report is still written first and the workflow fails afterward if missing keys are found. This keeps the final demo-vs-v2 proof inside BigQuery instead of pulling large tables into Python memory.
