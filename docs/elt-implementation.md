...
- `.github/workflows/serp-relevance-batch.yml` turns either a private-data facility CSV or BigQuery recent `fact_reviews` into a bounded SERP matrix, then stores each response with the same raw object contract.
